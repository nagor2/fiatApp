...
// Используем nginx proxy для избежания CORS проблем в production
// В dev режиме можно переопределить через REACT_APP_RPC_URL в .env
const defaultRpcUrl = "/api/rpc";
const rpcUrl = process.env.REACT_APP_RPC_URL || defaultRpcUrl;

// Web3 требует полный URL, конвертируем относительный путь в абсолютный
